...
**Deployed chains.** Monad testnet (10143): EligibilityRegistry, proof-gated PolicyRegistry, VerifiedAssetToken, SettlementToken, and DistributionEngine — five contracts with eight successful public receipts in docs/deployments.md. `anchorProof` is mechanically required before `enact`; the permissionless `IPreEnactmentProof` interface is consumed by the issuer, a protocol contract, the holder verifier, and an agent. Web console: https://meridian-three-olive.vercel.app.

**Evidence.** 24 unit + 37 Foundry tests · 500/500 differential vectors · 24/24 e2e checks (fixture AND live-chain) · 9/9 live Cooperate smoke · 9/9 self-audit surfaces · zero console errors at desktop and mobile widths.
...
|---|---|---|---|---|
| 1 | 0:00–0:08 | Veo clip `media/veo-act1.mp4` ([full prompt](prompts/videos.md)) — or fallback: black frame, slow fade-in of the landing hero | "March, twenty twenty-four. A tokenized fund discovers that six percent of its holders became sanctions-exposed… four weeks ago. Nobody saw it happen — because compliance was enacted blind." | Low, deliberate. Pause after "four weeks ago." |
...
| 6 | 1:10–1:28 | Click **Run sweep**. THE SIGNATURE SHOT: wavefront crosses, nodes flip, counters settle. Hold 2s after settle. Mouse to "Value stranded" card. | "Sweep. Forty-eight holders and every pending coupon, re-evaluated under the exact five-dimensional rule semantics the chain enforces. Thirteen holders strand. Three hundred sixty-nine thousand, nine hundred twenty-nine dollars loses transferability. Six Friday coupons would fail. You know the blast radius — before it's law." | This is the money shot; use the deterministic seed and confirm the displayed cents before recording. |
...
| 9 | 1:58–2:20 | Navigate to **Distributions**. Click **Execute run**. Table resolves: six paid rows green, six suspended amber with reasons. Click **Release** on a suspended leg → red refusal appears. Then **Remediate** → **Release** → row flips to released; escrow stat drops to zero for that leg. | "Mid-flight money doesn't fail — it's caught. The coupon run pays six legs and suspends six into on-chain escrow, each with its reason. Release is refused — on-chain — until the holder re-verifies through Cleanverse's own magic-link flow. Then the chain re-proves eligibility… and pays. Money caught. Not lost." | The refusal is a FEATURE — VO must sound satisfied, not apologetic. |
...
| 12 | 2:56–3:10 | Terminal, big font. Run `pnpm judge`; hold on the 9/9 scoreboard showing 24 unit, 37 Foundry, 500 vectors, and 24/24 e2e. | "Under the hood: nine of nine live calls against the Cleanverse sandbox. Thirty-seven contract tests. And five hundred differential vectors proving our simulator and the on-chain gate can never disagree." | Fast cuts, terminal beats land on VO numbers. |
...
4. Vercel from repo root: `VERCEL_PROJECT_ID=prj_67rqrWL6uTCCyGNOGsr9RRN9AiKL VERCEL_ORG_ID=team_vJQ3cdBoBM7afSLKkbooSs5E pnpm dlx vercel@latest deploy --prod --yes`
...
| Contracts on Monad + deployments.md | ✅ five contracts and eight successful public receipts |
