...
|---|---|---|---|
| 1 | Unit tests (Cleanverse client AES + lifecycle, sweep engine, seeded book) | **24 passing** | `pnpm vitest run` |
| 2 | Foundry unit suite (proof gate, public consumer, transfer gate, escrow, registry, policy chain, invariants) | **37 passing** | `cd contracts && forge test` |
...
| 8 | Proof-gated primitive deployed on Monad testnet (10143), 5 contracts | **8 successful receipts · blocks 51885991–51886029** | `docs/deployments.md` (address + tx links) |
| 9 | End-to-end demo path | **24/24 checks** (fixture AND live-chain) | `node --import tsx server/scripts/e2e-demo-path.ts` |
| 10 | Self-judging repo | **9/9 surfaces green** | `pnpm judge` |
...
| Live e2e lifecycle | — | — | 3 trades | full | full | **24/24 checks, real txids + exact active proof digest** |
...
- Deployed addresses and all eight receipts are linked on the explorer; the active proof is publicly readable and consumed, while the identical full lifecycle is reproduced on a fresh Anvil chain with real transaction hashes.
