...
export function ProvenanceChip({ source, chain }: { source: Provenance | "live"; chain?: boolean }) {
  const cfg =
    source === "live"
      ? chain
      ...
      : source === "fixture"
        ? { label: "SIMULATED · FIXTURE", color: "var(--sim-1)", bg: "var(--sim-bg)" }
        : { label: "SIMULATED · DEMO", color: "var(--sim-1)", bg: "var(--sim-bg)" };
  return (
    <span
      className="num inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-medium tracking-[0.06em]"
      style={{ color: cfg.color, background: cfg.bg }}
      title={source === "live" ? "This panel reflects live external state" : "This panel is an honestly-labeled simulation"}
    >
...
export function StatCard({ label, value, sub, tone }: { label: string; value: string; sub?: string; tone?: "ok" | "warn" | "danger" }) {
  const color = tone === "danger" ? "var(--danger-1)" : tone === "warn" ? "var(--warn-1)" : tone === "ok" ? "var(--ok-1)" : "var(--ink-1)";
  return (
    <div className="panel px-5 py-4">
      <div className="label">{label}</div>
      <div className="num mt-1.5 text-2xl font-medium" style={{ color }}>
        {value}
      </div>
...
export function Button({
  ...
  const styles: Record<string, React.CSSProperties> = {
    default: { background: "var(--bg-2)", color: "var(--ink-1)", boxShadow: "var(--elev-1)" },
    primary: { background: "var(--brand-1)", color: "#04121a", boxShadow: "var(--elev-glow)" },
    danger: { background: "var(--danger-bg)", color: "var(--danger-1)", boxShadow: "0 0 0 1px rgba(248,113,113,0.3)" },
...
export function Verdict({ reason, label }: { reason: number; label: string }) {
  const tone = reason === 0 ? { c: "var(--ok-1)", b: "var(--ok-bg)" } : reason === 3 || reason === 4 ? { c: "var(--warn-1)", b: "var(--warn-bg)" } : { c: "var(--danger-1)", b: "var(--danger-bg)" };
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] font-medium" style={{ color: tone.c, background: tone.b }}>
  ...
  return (
    <div className="panel flex flex-col items-center justify-center gap-2 px-8 py-16 text-center">
      <svg width="28" height="28" viewBox="0 0 64 64" fill="none" opacity={0.4}>
        ...
        <path d="M32 6 L32 20 M32 44 L32 58" stroke="var(--ink-3)" strokeWidth="4" strokeLinecap="round" />
      </svg>
